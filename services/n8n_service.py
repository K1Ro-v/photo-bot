...
async def process_n8n_request(user_id: int, prompt: Optional[str], settings: dict,
    ...
    try:
        ...
        if n8n_response and n8n_response.get('success'):
            await context.bot.send_message(chat_id=chat_id, text="✅ Готово! Твой футуристичный портрет готов! 🚀\n\nМожешь отправить новое фото для обработки!")
        else:
            await context.bot.send_message(chat_id=chat_id, text="❌ Упс! Не удалось создать портрет... Попробуй еще раз! 🤖\n\nВозможно, произошла временная ошибка.")

    except Exception as e:
        print(f"❌ Error in background task for user {user_id}: {e}")
        try:
            await context.bot.send_message(chat_id=chat_id, text="⚠️ Ой! Что-то пошло не так... Попробуй снова!\n\nAI иногда может давать сбой, но мы работаем над этим! 🔧")
        except:
            pass
